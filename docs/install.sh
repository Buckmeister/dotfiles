...
#     curl -fsSL https://buckmeister.github.io/install.sh | sh
#
#   Automatic (installs everything):
#     curl -fsSL https://buckmeister.github.io/install.sh | sh -s -- --auto
#
# This script:
# - Detects OS (macOS, Linux, WSL)
# - Checks for required tools (git, zsh)
# - Clones the dotfiles repository
# - Runs setup (interactive menu or automatic with --auto flag)
#
...
# Installation mode (interactive by default, --auto for automatic)
...
Options:
  --auto    Automatic installation with all modules (non-interactive)
  --help    Show this help message

Default: Interactive installation with menu system

Examples:
  # Interactive installation (launches menu)
  curl -fsSL https://buckmeister.github.io/install.sh | sh

  # Automatic installation (installs everything)
  curl -fsSL https://buckmeister.github.io/install.sh | sh -s -- --auto
EOF
            exit 0
            ;;
...
main() {
    ...
    if [ -f "$SETUP_SCRIPT" ]; then
        ...
        # Determine setup arguments based on installation mode
        if [ "$INSTALL_MODE" = "auto" ]; then
            print_info "Running automatic installation with all modules..."
            SETUP_ARGS="--all-modules"
        else
            print_info "Launching interactive menu..."
            SETUP_ARGS=""
        fi

        # Run setup
        if check_command zsh; then
            zsh "$SETUP_SCRIPT" $SETUP_ARGS
        else
            sh "$SETUP_SCRIPT" $SETUP_ARGS
        fi
    else
